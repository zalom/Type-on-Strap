...
echo "=================================================="
echo "Simple-Jekyll-Search Validation (v2.1.0)"
echo "=================================================="
echo ""
...
if [ -n "$VERSION" ]; then
    ...
    if [ "$VERSION" = "v2.1.0" ]; then
        echo -e "${GREEN}✅ Using expected version (v2.1.0)${NC}"
    else
        echo -e "${YELLOW}⚠️  Version mismatch - expected v2.1.0, found ${VERSION}${NC}"
    fi
else
